...
Alpine.data('terminalPortfolio', (config = {}) => ({
  ...
  loadCommands() {
    ...
    this.$root.querySelectorAll('template[data-terminal-file]').forEach((template) => {
      this.files[template.dataset.terminalFile] = template.innerHTML.trim();
    });
  },
  ...
  resolveCommand(command, args) {
    ...
    if (command === 'pwd') {
      return `<div class="mb-4 text-slate-800 dark:text-slate-300">/home/${this.prompt.user}</div>`;
    }

    if (command === 'cat') {
      const name = args[1] || '';
      if (this.files[name]) {
        return `<div class="mb-4 text-slate-800 dark:text-slate-300">${this.files[name]}</div>`;
      }
      return `<div class="mb-4 text-red-400">cat: ${this.escapeHtml(name || 'missing operand')}: No such file. Try 'ls'.</div>`;
    }
