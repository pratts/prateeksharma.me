...
tech: [Go]
code: https://github.com/pratts/goroomlib
related:
  - /experience/adda52/
  - /experience/mind-sports-league/
---

Across the poker platforms I've worked on at Adda52 and Mind Sports League, one
shape kept showing up: users joining a table, leaving it, sending messages to
everyone at it, and the server keeping track of who's where. Every time I touched
...
1. Let a consumer create rooms and users without worrying about the bookkeeping
...
backed by a map protected by a mutex. The third one needed more thought.
...
one of those two updates to land while the other was still in flight, leaving the
two data structures briefly (or not so briefly) out of sync.

The fix was to make membership changes atomic: `AddUserToRoom` and
`RemoveUserFromRoom` now acquire the lock once and update both sides of the
relationship before releasing it. Nothing else is allowed to observe a
half-updated state.
...
I made room names unique across the whole system rather than introducing a
separate handle for lookups. `CreateRoom` returns `nil` if the name is already
taken, and `IsRoomNameAvailable` lets a caller check before attempting creation:
...
It's a simple constraint, but it meant I didn't need to design a separate
ID-vs-name lookup story, and callers get an obvious, explicit failure mode
instead of silently overwriting an existing room.
...
domain-specific behavior, and the room/user lifecycle is already handled and safe
