...
- `WazirxClientException`: the caller made a mistake the caller can fix, such as
  a bad `side` value or a missing required field. Thrown before a request is
  even sent.
- `WazirxApiException`: something went wrong once the request left the process,
  such as a network failure or a non-2xx response.
